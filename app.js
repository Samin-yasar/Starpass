...
function generatePassword() {
    ...
    // Validate inputs
    if (
        !validateInput(lowercaseCount, 0) ||
        !validateInput(uppercaseCount, 0) ||
        !validateInput(numbersCount, 0) ||
        !validateInput(specialCount, 0)
    ) {
        errorDiv.textContent = 'Please enter valid numbers';
        outputDiv.classList.add('hidden');
        return;
    }
    if (lowercaseCount + uppercaseCount + numbersCount + specialCount < 8 || totalLength > 50) {
...
// Clipboard copy action with feedback
